...
class Kernel(object):
    ...
    def bounds(self, a, point):
        ...
        if point.x < self.offset:
            ...
            m = self.matrix[
                :, 0:self.matrix.shape[1] + (x_max - point.x - self.offset) - 1]
            use_m = True
        else:
            x_start = point.x - self.offset
            x_end = point.x + self.offset + 1
    ...
    def apply(self, a, point, source=None, f=None, v=None):
        ...
        # print a.shape, point, x_start, x_end, y_start, y_end, (m if use_m
        # else self.matrix).shape
        if m is not False:
            a[y_start:y_end, x_start:x_end] = f(
                source[y_start:y_end, x_start:x_end], m)
        else:
            raise OutOfBounds(
...
class ConstantKernel(Kernel):
    ...
    def __init__(self, size=1, value=None):
        ...
        if value:
            ...
            # Normalize the sum of all cells in the matrix to 1
            self.matrix /= sum(self.matrix)
...
class ArrayKernel(Kernel):
    ...
    def __init__(self, a, const=None):
        ...
        b = np.pad(
            a, ((0, pad_y), (0, pad_x)), 'constant', constant_values=(
                (0, 0), (0, 0)))  # @UndefinedVariable
