...
# An inverted map -- values are unique. It really should have the keys and
# values reversed, but this is less work
ogr_inv_type_map = {
...
def copy_schema(schema, path, table_name=None, fmt='shapefile', logger=None):
    ...
    if ds.GetLayerCount() > 1 and table_name:
        raise ValueError(
            "Can't specify table_name for a file with multiple layers")
...
class TableShapefile(object):
    ...
    def add_feature(self, row, source_srs=None):
        ...
        if self.layer is None:
            ...
            if self.layer is None:
                raise Exception(
                    "Failed to create layer {} in {}".format(
                        self.name,
                        self.path))
