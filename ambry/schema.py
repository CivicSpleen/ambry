...
class Schema(object):
    ...
    def tables(self):
        ...
        q = (
            self.bundle.database.session.query(Table).filter(
                Table.d_vid == self.d_vid))
    ...
    def add_table(self, name, add_id=False, **kwargs):
        ...
        col_data = {
            k.replace(
                ...
                '',
                1): v for k,
            v in kwargs.items() if k.startswith('d_')}
    ...
    def add_column(self, table, name, **kwargs):
        ...
        try:
            ...
            raise ConfigurationError(
                "Sequence id value '{}' is not an integer in table '{}' col '{}'" .format(
                    kwargs['sequence_id'],
                    table.name,
                    name))

        sequence_id = int(kwargs['sequence_id']) if 'sequence_id' in kwargs and kwargs[
            'sequence_id'] is not None else None
        ...
        if sequence_id is None:
            ...
            raise ConfigurationError(
                "Column '{}' specifies column number '{}', but last number in table '{}' is {}" .format(
                    name,
                    sequence_id,
                    table.name,
                    self.max_col_id[
    ...
    def validate_column(cls, table, column, warnings, errors):
        ...
        if column.datatype == Column.DATATYPE_TEXT and column.size:
            warnings.append(
                (table.name,
                 column.name,
                 "Postgres doesn't allow a TEXT field to have a size. Use a VARCHAR instead."))

        # MySql requires that text columns that have a default also have a
        # size.
        if column.type_is_text() and bool(column.default):
            if not column.size and not column.width:
                warnings.append(
                    (table.name,
                     column.name,
                     "MySql requires a Text or Varchar field with a default to have a size."))
        ...
        if column.default:
            try:
                ...
                errors.append(
                    (table.name,
                     column.name,
                     "Bad default value '{}' for type '{}' (T); {}".format(
                         column.default,
                         column.datatype,
                         e)))
            except ValueError:
                errors.append(
                    (table.name,
                     column.name,
                     "Bad default value '{}' for type '{}' (V)".format(
                         column.default,
                         column.datatype)))
    ...
    def translate_type(cls, driver, table, column):
        ...
        if driver == 'mysql':
            ...
            if (column.datatype in (Column.DATATYPE_TEXT, column.datatype == Column.DATATYPE_VARCHAR) and
                ...
                raise ConfigurationError(
                    "Bad column {}.{}: For MySql, text columns with default must also have size or width" .format(
                        table.name,
                        column.name))
    ...
    def get_table_meta_from_db(
        ...
        table = self.get_table_from_database(
            db,
            name_or_id,
            d_vid=d_vid,
            session=session)
        ...
        for column in table.columns:
            ...
            width = column.size if column.size else (
                column.width if column.width else None)
    ...
    def caster(self, table_name):
        ...
        for c in table.columns:
            ...
            if 'caster' in c.data and c.data['caster']:
                ...
                if not t:
                    self.bundle.error(
                        "Schema declared undefined caster type {} for {}.{}. Ignoring" .format(
                            c.data['caster'],
                            table.name,
                            c.name))
                    t = c.python_type
            ...
            for row in reader:
                ...
                if new_table and row['table']:
                    ...
                    try:
                        ...
                        errors.append(
                            (None,
                             None,
                             " Failed to add table: {}. Row={}. Exception={}".format(
                                 row['table'],
                                 dict(row),
                                 e)))
                        return warnings, errors
                ...
                data = {
                    k.replace(
                        ...
                        '',
                        1): v for k,
                    v in row.items() if k.startswith('d_')}
    ...
    def expand_table_prototypes(self):
        ...
        for t, proto_vid in table_protos():
            ...
            try:
                ...
                self.bundle.error(
                    "Can't expand prototype for table {}: missing prototype reference {} (a)" .format(
                        t.name,
                        proto_vid))
                continue
            ...
            if not proto_table:
                self.bundle.error(
                    "Can't expand prototype for table {}: missing prototype reference {} (b)" .format(
                        t.name,
                        proto_vid))
                continue
            ...
            for c in proto_table.columns:
                ...
                if c.name == 'id':
                    # This column already exists, not re-adding it preserves
                    # numbering
                    idc = t.column('id')
                    ...
                    # Protovids never have the version -- they arent really
                    # vids
                    d['proto_vid'] = c.id_
    ...
    def expand_column_prototypes(self):
        ...
        q = (
            self.bundle.database.session.query(Column).filter(
                Column.proto_vid is not None))
        ...
        for c in q.all():
            ...
            try:
                ...
                if not pt_map:
                    p = self.bundle.library.get(
                        'civicknowledge.com-proto-proto_terms').partition
        ...
        if os.path.exists(fsp(sb.SCHEMA_FILE)):
            ...
            try:
                if not filecmp.cmp(fsp(sb.SCHEMA_REVISED_FILE), fsp(sb.SCHEMA_FILE)):
                    shutil.copy(
                        fsp(sb.SCHEMA_REVISED_FILE), fsp(sb.SCHEMA_FILE))
            except OSError:  # hopefully only a file not found error
                pass
    ...
    def _dump_gen(self, table_name=None):
        ...
        if table_name:
            tables = [
                table for table in self.tables if table.name == table_name]
        else:
            tables = self.tables

        data_fields = set()
        # Need to get all of the indexes figured out first, since there are a
        # variable number of indexes.
        for table in tables:
        ...
        opt_col_fields = [
            field for field in all_opt_col_fields if field in opt_fields_set]
    ...
    def as_text(self, table, pad='    '):
        ...
        for row in rows:
            ...
            if row[-1]:
                # Put in the first row of the wrapped desc
                drows = textwrap.wrap(row[-1], desc_wrap)
                row[-1] = drows.pop(0)
            else:
                drows = []
            ...
            if drows:
                # Get rid of the desc, so we can get the length of the padding
                # for subsequent rows.
                row.pop()
                dsizes = list(sizes)
                dsizes.pop()
        ...
        def write_fields(table):
            ...
            for col in table.columns:
                ...
                o += "    {column} = SAColumn('{column}',sqlalchemy.types.{type}{options})\n".format(
                    column=col.name,
                    type=col.sqlalchemy_type.__name__,
                    options=optstr)
...
"""
        ...
        with self.bundle.session as s:
            ...
            for c in table.columns:
                ...
                if size and size > c.size:
                    ...
                    # Integers that are too long for 32 bits should be upgraded
                    # to 64
                    # 2^32 is 10 gigits
                    if c.datatype == c.DATATYPE_INTEGER and c.size >= 10:
                        self.bundle.log(
    ...
    def extract_columns(self, extract_table, extra_columns=None):
        ...
        for col in et.columns:
            ...
            lines.append(
                "CAST({sql} AS {type}) AS {col}".format(
                    sql=sql,
                    col=col.name,
        ...
        with self.bundle.session as s:
            ...
            for col in intuiter.columns:
                ...
                if name == 'id':
                    ...
                    if has_codes:
                        if False:
                            # This mostly just muchs up loading files be
                            # altering the header.
                            self.add_column(
                                table,
                                name + '_codes',
