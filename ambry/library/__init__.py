...
def new_library(config=None, reset=False):
    """Return a new :class:`~ambry.library.Library`,
    constructed from a configuration
...
class Library(object):
    ...
    def __init__(self, cache, database,
        ...
        self.host = host
        self.port = port
        self.urlhost = urlhost if urlhost else (
            '{}:{}'.format(
                self.host,
        ...
        if not self.cache:
            raise ConfigurationError(
                "Must specify library.cache for the library in bundles.yaml")
    ...
    def put_bundle(
        ...
        self.files.install_bundle_file(
            bundle,
            self.cache,
            commit=commit,
            state='new')
    ...
    def put_partition(self, bundle, partition, commit=True):
        ...
        installed = self.files.install_partition_file(
            partition,
            self.cache,
            commit=commit,
            state='new')

        # Ref partitions use the file of an earlier version, so there is no
        # FILE to install
    ...
    def list(self, datasets=None, with_partitions=False):
        ...
        Does not include partitions. This returns a dictionary in  a
        form that is similar to the remote and source lists.
            ...
            for ident in sorted(self.list().values(), cmp=rev_cmp):
                ...
                if not current or ident.id_ != current.identity.id_:
                    ...
                    try:
                        ...
                        # This happens frequently in warehosues, where
                        # only one version of the dataset is installed.
                        pass
                else:
    ...
    def _get_bundle_by_cache_key(self, cache_key, cb=None):
        ...
        # The AltReadCache will get from the remote stack if the path is not
        # found in the cache.
        ...
        try:
            ...
            self.logger.error(
                "Failed to load databundle at path {}: {}".format(
                    abs_path,
                    e))
            raise
    ...
    def get(self, ref, force=False, cb=None, location='default'):
        ...
        if not dataset:
            raise NotFoundError(
                "Failed to resolve reference '{}' in library '{}' ".format(
                    ref,
                    self.database.dsn))
        ...
        if not bundle:
            raise NotFoundError(
                "Failed to get bundle from cache key: '{}'".format(
                    dataset.cache_key))
        ...
        try:
            if dataset.partition:
                ...
                if not partition:
                    ...
                    raise NotFoundError(
                        'Failed to get partition {} from bundle at {} '
                                .format(dataset.partition.fqname,
                                        dataset.cache_key))
                ...
                if partition.ref:
                    ...
                    if not ref_ident:
                        raise NotFoundError(
                            "Reference '{}' refers to another partition, "
                    ...
                    if self.cache.has(ref_partition_ident.cache_key):
                        # The referenced partition already exists, so it
                        # should be copied to the refferent path
                        delete = False

                    else:
                        # The referenced partition does not exist, so it
                        # should be moved to the refferent path. BUT, moving
                        # is not really supported by the caches, so we will
                        # copy then delete.

                        delete = True

                    ref_abs_path = arc.get(
                        ref_partition_ident.cache_key,
                        cb=cb)

                    abs_path = self.cache.put(
                        ref_abs_path,
                        partition.cache_key)
                ...
                if not abs_path or not os.path.exists(abs_path):
                    raise NotFoundError(
                        'Failed to get partition {} from cache '.format(
                            partition.identity.cache_key))

                try:
                    self.database.install_partition_by_id(
                        bundle,
                        dataset.partition.id_,
    ...
    def tables_no_columns(self):
        ...
        return self.database.session.query(
            Table).options(lazyload('columns')).all()
    ...
    def table(self, vid):
        ...
        try:
            return (
                self.database.session.query(Table).filter(
                    Table.vid == vid).one())
            ...
            try:
                return (
                    self.database.session.query(Table) .filter(
                        Table.id_ == vid).order_by(
                        Table.vid.desc()).first())
            except NoResultFound:
                raise NotFoundError(
                    "Did not find table ref {} in library {}".format(
                        vid,
                        self.database.dsn))
    ...
    def derived_tables(self, proto_vid):
        ...
        try:
            return (
                self.database.session.query(Table).filter(
                    Table.proto_vid == proto_vid).all())
        except NoResultFound:
            raise NotFoundError(
                "Did not find table with proto_vid {} in library {}" .format(
                    proto_vid,
                    self.database.dsn))
    ...
    def dataset(self, vid):
        ...
        try:
            ...
            try:
                ds = self.database.session.query(Dataset).filter(
                    Dataset.id_ == vid).order_by(
                ...
                from ..dbexceptions import NotFoundError
                raise NotFoundError(
                    "Failed to find dataset for ref '{}' ".format(vid))
    ...
    def versioned_datasets(self):
        """Like datasets(), but returns a dict structure, and only the most
        recent version, with other versions under the 'other_version' key."""
        from ..orm import Dataset

        datasets = {}

        for ds in (self.database.session.query(Dataset)
    ...
    def partition(self, vid):
        ...
        try:
            return (
                self.database.session.query(Partition).filter(
                    Partition.vid == vid).one())
        except NoResultFound:
            try:
                return (
                    self.database.session.query(Partition).filter(
                        Partition.id_ == vid).one())
            except NoResultFound:
                self.logger.error(
                    "No partition found: {} for {}".format(
                        vid,
                        self.database.dsn))
                raise NotFoundError(
                    "No partition in library for vid : {} ".format(vid))
    ...
    def store(self, uid):
        ...
        if not f:
            f = self.files.query.group(
                self.files.TYPE.STORE).path(uid).one_maybe
    ...
    def warehouse(self, uid):
        ...
        if not s:
            from ..dbexceptions import NotFoundError
            raise NotFoundError(
                "Did not find warehouse for uid: '{}' ".format(uid))
    ...
    def remove_manifest(self, uid):
        ...
        if not f:
            raise NotFoundError(
                "Didn't find manifest for uid '{}' ".format(uid))
    ...
    def resolve(self, ref, location='default'):
        ...
        # If the location is not explicitly defined, set it to everything but
        # source
        if location is 'default':
            location = [
                LocationRef.LOCATION.LIBRARY,
                LocationRef.LOCATION.PARTITION,
    ...
    def locate(self, ref):
        """Return list of files for a reference, indicating where a file for a
        partition or dataset is located."""
    ...
    def locate_one(self, ref):
        """Like locate, but return only the highest priority result,
        or None if non exists."""
    ...
    def dep(self, name):
        """"Bundle version of get(), which uses a key in the bundles
        configuration group 'dependencies' to resolve to a name."""
        ...
        if not o:
            raise DependencyError(
                "Failed to get dependency, key={}, id={}".format(
                    name,
                    object_ref))
    ...
    def _get_dependencies(self):
        ...
        if not self._bundle:
            raise ConfigurationError(
                "Can't use the dep() method for a library that is not "
        ...
        for k, v in self.dependencies.items():
            ...
            if download:
                ...
                if not b:
                    if throw:
                        raise NotFoundError(
                            "Dependency check failed for key={}, id={}. "
                            "Failed to get bundle or partition".format(
                                k,
                                v))
                    else:
                        errors[k] = v
    ...
    @property
    def source(self):
        """Return a SourceTree object, based on the source_repo
        configuration."""
    ...
    def push(self, ref=None, cb=None, upstream=None):
        ...
        if ref is not None:
            ...
            if not dsid:
                raise Exception(
                    "Didn't get id from database for ref: {}".format(ref))
    ...
    def purge(self):
        """Remove all records from the library database, then delete all files
        from the cache."""
        self.clean()
        self.cache.clean()
    ...
    def mark_updated(self, o=None, vid=None):
        """Mark an object as recently updated, for instance to clear the
        doc_cache."""

        self.doc_cache.remove(vid)

    def sync_library(self, clean=False):
        """Rebuild the database from the bundles that are already installed in
        ...
        for r, d, f in os.walk(self.cache.cache_dir, topdown=True):
            ...
            for file_ in f:
                ...
                if file_.endswith(".db"):
                    ...
                    extant_bundle = self.files.query.type(
                        Files.TYPE.BUNDLE).path(path_).one_maybe
                    extant_partition = self.files.query.type(
                        Files.TYPE.PARTITION).path(path_).one_maybe
                    ...
                    try:
                        ...
                        try:
                            ...
                            self.logger.error(
                                "Failed to open bundle from {}: {} ".format(
                                    path_,
                                    e))
                            continue

                        # The path check above is wrong sometime when there are
                        # symlinks
                        if ( self.files.query.type(Files.TYPE.BUNDLE)
                                     .ref(bident.vid).one_maybe
                             and self.get(bident.vid)):
                            continue
            ...
            try:
                ...
                try:
                    ...
                    for p in bundle.partitions:
                        ...
                        if installed:
                            self.database.install_partition(
                                bundle,
                                p,
                                commit='collect')

                        self.files.install_partition_file(
                            p,
                            self.cache,
                            commit='collect')
                ...
                self.logger.error(
                    'Failed to install bundle {}: {}'.format(
                        bundle.identity.vname,
                        e.message))
                raise
                continue
    ...
    def sync_remotes(
            ...
            last_only=True,
            vids=None):
        """Sync the local library with all of the remotes, create static
        JSON, and  build the full-text search index"""
        from ..orm import Dataset
        from sqlalchemy.exc import IntegrityError
        ...
        for remote in remotes:
            ...
            all_keys = [
                f.path for f in self.files.query.type(
                    Dataset.LOCATION.REMOTE).group(
                    remote.repo_id).all]
            ...
            if last_only:
                ...
                for cache_key in remote_list:
                    nv_key = re.sub(
                        r'-\d+\.\d+\.\d+\.db',
                        '',
                        cache_key)  # Key without the version
            ...
            for cache_key in remote_list:
                ...
                if use_only and cache_key not in use_only:
                    self.logger.info("Skip old version: ({}"
                                     .format(cache_key))
                    continue

                if self.cache.has(cache_key):  # This is just for reporting.
                    self.logger.info("Remote {} has: {}"
                ...
                if not b:
                    self.logger.error(
                        "Failed to fetch bundle for {} ".format(cache_key))
                    continue
                ...
                try:
                    path, installed = self.put_bundle(
                        b, install_partitions=False, commit=True)

                except NotABundle:
                    self.logger.error(
                        "Cache key {} exists, but isn't a valid bundle"
                        .format(cache_key))
                    b.close()
                    continue
                except Exception as e:
                    self.logger.error(
                        "Failed to put bundle {}: {}".format(
                            cache_key,
                            e))
                    b.close()
                    raise
                ...
                try:
                    self.files.install_remote_bundle(
                        b.identity,
                        remote,
                        {},
                        commit=True)
                ...
                for p in b.partitions:
                    ...
                    if ( self.files.install_remote_partition(
                            p.identity, remote, {}, commit='collect')):
    ...
    def sync_source(self, clean=False):
        """Rebuild the database from the bundles that are already
        installed in the repository cache."""
    ...
    def sync_warehouse(self, w):
        """Create a reference to the warehouse and link all of the partitions
        to it."""
        ...
        for remote_manifest in w.manifests:
            ...
            # Copy the file record. There really should be an easier way to do
            # this.
            ...
            # when the links are one-directional.
            local_manifest.data[
                'tables'] = remote_manifest.data.get('tables', [])
