"""A Library is a local collection of bundles. It holds a database for the
configuration of the bundles that have been installed into it.
"""
...
class Library(object):
    ...
    def __init__(self, cache, database,
        ...
        '{}:{}'.format(self.host, self.port) if self.port else self.host)
    ...
    def _get_bundle_by_cache_key(self, cache_key, cb=None):
        ...
        try:
            ...
            self.logger.error(
                "Failed to load databundle at path {}: {}".format(abs_path, e))
            raise
    ...
    def get(self, ref, force=False, cb=None, location='default'):
        ...
        if not dataset:
            raise NotFoundError(
                "Failed to resolve reference '{}' in library '{}' ".format(ref,
                                                                           self.database.dsn))
        ...
        try:
            if dataset.partition:
                ...
                if partition.ref:
                    ...
                    if not ref_ident:
                        raise NotFoundError(
                            "Reference '{}' refers to another partition, '{}', which does not exist"
                ...
                try:
                    self.database.install_partition_by_id(bundle,
    ...
    def tables_no_columns(self):
        ...
        return self.database.session.query(Table).options(
            lazyload('columns')).all()
    ...
    def table(self, vid):
        ...
        try:
            return (
            self.database.session.query(Table).filter(Table.vid == vid).one())
        except NoResultFound:
            # Ths vid is actually an id, so we take the latest one
            try:
                return (self.database.session.query(Table)
                        .filter(Table.id_ == vid).order_by(
                    Table.vid.desc()).first())
            except NoResultFound:
                raise NotFoundError(
                    "Did not find table ref {} in library {}".format(vid,
                                                                     self.database.dsn))
    ...
    def derived_tables(self, proto_vid):
        ...
        try:
            return (self.database.session.query(Table).filter(
                Table.proto_vid == proto_vid).all())
        except NoResultFound:
            raise NotFoundError(
                "Did not find table with proto_vid {} in library {}"
    ...
    def dataset(self, vid):
        ...
        try:
            return (self.database.session.query(Dataset).filter(
                Dataset.vid == vid).one())
        except NoResultFound:
            try:
    ...
    def versioned_datasets(self):
        ...
        for ds in (self.database.session.query(Dataset).order_by(
                Dataset.revision.desc()).all()):
    ...
    def partition(self, vid):
        ...
        try:
            return (self.database.session.query(Partition).filter(
            ...
            try:
                ...
                self.logger.error("No partition found: {} for {}".format(vid,
                                                                         self.database.dsn))
                raise NotFoundError(
    ...
    def store(self, uid):
        ...
        if not f:
            f = self.files.query.group(self.files.TYPE.STORE).path(
                uid).one_maybe
    ...
    def resolve(self, ref, location='default'):
        ...
        if location is 'default':
            location = [LocationRef.LOCATION.LIBRARY,
    ...
    def dep(self, name):
        ...
        if not o:
            raise DependencyError(
                "Failed to get dependency, key={}, id={}".format(name,
                                                                 object_ref))
        ...
        for r, d, f in os.walk(self.cache.cache_dir, topdown=True):
            ...
            for file_ in f:
                ...
                if file_.endswith(".db"):
                    ...
                    extant_partition = (self.files.query
                                        .type(Files.TYPE.PARTITION)
                                        .path(path_).one_maybe )
            ...
            try:
                ...
                try:
                    ...
                    for p in bundle.partitions:
                        self.logger.info('            {} '
                                         .format(p.identity.vname))

                        if installed:
                            self.database.install_partition(bundle, p,
                                                            commit='collect')

                        self.files.install_partition_file(p, self.cache,
                                                          commit='collect')
    ...
    def sync_remotes(self, remotes=None, clean=False,
        ...
        for remote in remotes:
            ...
            for cache_key in remote_list:
                ...
                if not b:
                    self.logger.error("Failed to fetch bundle for {} "
                                      .format(cache_key))
                    continue
                ...
                try:
                    path, installed = self.put_bundle(b,
