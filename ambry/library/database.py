...
class LibraryDb(object):
    ...
    DBCI = {
        'postgis': Dbci(
            dsn_template='postgresql+psycopg2://{user}:{password}@{server}{colon_port}/{name}',
            sql='support/configuration-pg.sql'),
        'postgres': Dbci(
            dsn_template='postgresql+psycopg2://{user}:{password}@{server}{colon_port}/{name}',
            sql='support/configuration-pg.sql'),  # Stored in the ambry module.
        'sqlite': Dbci(
            dsn_template='sqlite:///{name}',
            sql='support/configuration-sqlite.sql'),
        'spatialite': Dbci(
            dsn_template='sqlite:///{name}',
            sql='support/configuration-sqlite.sql'),
        'mysql': Dbci(
            dsn_template='mysql://{user}:{password}@{server}{colon_port}/{name}',
            sql='support/configuration-sqlite.sql')
    }
        ...
        try:
            try:
                ...
                rows = self.connection\
                    .execute(
                        "SELECT * FROM datasets WHERE d_vid = '{}' ".format(ROOT_CONFIG_NAME_V))\
                    .fetchone()
    ...
    def clone(self):
        return self.__class__(
            self.driver, self.server, self.dbname,
    ...
    def get_config_value(self, group, key):
        ...
        try:
            ...
            c = s.query(SAConfig)\
                .filter(SAConfig.group == group, SAConfig.key == key,
                        SAConfig.d_vid == ROOT_CONFIG_NAME_V)\
                .first()
    ...
    def get_config_group(self, group, d_vid=ROOT_CONFIG_NAME_V):
        ...
        try:
            d = {}
            for row in s.query(SAConfig).filter_by(group=group, d_vid=d_vid).all():
                d[row.key] = row.value
            return d
    ...
    def get_config_rows(self, d_vid):
        ...
        configs = self.session.query(SAConfig)\
            .filter(
                or_(SAConfig.group == 'config', SAConfig.group == 'process'),
                SAConfig.d_vid == d_vid)\
            .all()
    ...
    def config_values(self):
        ...
        for config in s.query(SAConfig).filter_by(d_vid=ROOT_CONFIG_NAME_V).all():
    ...
    def install_bundle_dataset(self, bundle):
        ...
        if self.session.query(Dataset).filter_by(vid=str(bundle.identity.vid)).first():
            return False

        dataset = bundle.database.session.query(Dataset)\
            .options(noload('*'), joinedload('configs'))\
            .filter(Dataset.vid == str(bundle.identity.vid))\
            .one()
    ...
    def install_bundle(self, bundle):
        ...
        for tbl in [Table, Column, Code, Partition, ColumnStat]:
            ...
            if tbl in (Column, ColumnStat, Code):
                for r in rows:
                    for k, v in r.items():
    ...
    def mark_table_installed(self, table_or_vid):
        ...
        try:
            table = s.query(Table).filter_by(vid=table_or_vid).one()
        except NoResultFound:
            table = s.query(Table).filter_by(name=table_or_vid).one()
    ...
    def remove_bundle(self, bundle):
        ...
        dataset_identity = self.get(bundle.identity.vid)
        if not dataset_identity:
            return False

        dataset_instance = self.session.query(Dataset)\
            .filter_by(vid=dataset_identity.vid).one()
        self.session.delete(dataset_instance)
        self.commit()
    ...
    def delete_dataset_colstats(self, dvid):
        ...
        s.query(ColumnStat)\
            .filter(ColumnStat.p_vid.in_(part_query.subquery()))\
        ...
        try:
            ...
            if isinstance(vid, DatasetNumber):
                d = self.session.query(Dataset)\
                    .filter(Dataset.vid == str(vid))\
                    .one()
                did = d.identity

            elif isinstance(vid, PartitionNumber):
                d, p = self.session.query(Dataset, Partition)\
                    .join(Partition)\
    ...
    def all_vids(self):
        ...
        q = self.session.query(Dataset, Partition)\
            .join(Partition)\
            .filter(Dataset.vid != ROOT_CONFIG_NAME_V)

        for row in q.all():
            vids.add(row.Dataset.vid)
            vids.add(row.Partition.vid)
