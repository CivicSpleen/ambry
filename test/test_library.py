...
class Test(TestBase):
    ...
    def test_library_push(self):
        ...
        expected_dump = """source/dataset-subset-variation-0.0.1.db:
...
"""
        ...
        self.assertEquals(
            '/tmp/library-test/remote-cache-1/source/dataset-subset-variation-0.0.1.db',
            fn)
    ...
    def test_version_resolver(self):
        ...
        self.assertEquals(
            'source-dataset-subset-variation-0.0.1~diEGPXmDC8001',
            str(result))
        ...
        self.assertEquals(
            'source-dataset-subset-variation-1.10.1~diEGPXmDC8001',
            str(result))

        ip, result = r.resolve_ref_one('source-dataset-subset-variation->=1.10.1,<3.0.0')
        self.assertEquals(
            'source-dataset-subset-variation-2.20.2~diEGPXmDC8002',
            str(result))
        ...
        self.assertEquals(
            'source-dataset-subset-variation-3.30.3~diEGPXmDC8003',
            str(result))
    ...
    def x_test_remote(self):
        ...
        rc = get_runconfig(
            (os.path.join(self.bundle_dir, 'server-test-config.yaml'), RunConfig.USER_CONFIG))
    ...
    def test_partitions(self):
        ...
        # Install the partition references in the library.
        l.put_bundle(self.bundle)
    ...
    def test_files(self):
        ...
        for e in [(str(i), str(j)) for i in range(10) for j in range(3)]:
            ...
            f = l.files.new_file(
                path='path' + e[0],
                ref='{}-{}'.format(*e), group=e[1], type_=e[1])
