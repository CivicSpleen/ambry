...
class _RasterLayer(object):
    def __init__(self, aa, file_=None, data_type=GDT_Byte, nodata=0):
        ...
        self.ogr_ds = ogr.GetDriverByName('Memory').CreateDataSource(
...
def get_analysis_area(library, **kwargs):
    ...
    if not place:
        raise Exception(
            "Failed to get analysis area record for geoid: {}".format(code))
...
class AnalysisArea(object):
    ...
    def __init__(self, name, geoid,
        ...
        if self.easmin % self.MAJOR_GRID + self.eastmax % self.MAJOR_GRID + self.northmin % self.MAJOR_GRID + \
                self.northmax % self.MAJOR_GRID > 0:
            raise Exception("Bounding box dimensions must be even modulo {}".format(self.MAJOR_GRID))

    def _too_big(self):
        if self.size_x * self.size_y > self.MAX_CELLS:
            raise Exception("Too big for scale {}: {} * {} = {}M cells".format(self._scale, self.size_x, self.size_y,
                                                                               self.size_x * self.size_y / 1000000))
    ...
    def scale(self, scale):
        ...
        if self.MAJOR_GRID % self._scale != 0:
            raise Exception(
                "The _scale {} must divide evenly into the MAJOR_GRID {}".format(self._scale, self.MAJOR_GRID))
    ...
    def state(self):
        ...
        if r:
            ...
            raise NotImplementedError(
                "state can only handle AAs with census geoids. ")
    ...
    def is_in_ll(self, lon, lat):
        ...
        return self.latmin < lat < self.latmax and self.lonmin < lon < self.lonmax
    ...
    def is_in_utm(self, x, y):
        """Return true if the (lat, lon) is inside the area"""
        return self.northmin < y < self.northmax and self.eastmin < x < self.eastmax
    ...
    def write_poly(self, file_, layer='poly', poly=None):
        ...
        layer = datasource.CreateLayer(layer, srs=poly.GetSpatialReference(), geom_type=ogr.wkbPolygon)
    ...
    def get_geotiff(self, file_, bands=1, over_sample=1,
                    data_type=DEFAULT_D_TYPE, nodata=0):
        return self._get_image(file_, bands, over_sample, data_type, nodata,
                               'GTiff')

    def get_memimage(self, bands=1, over_sample=1, data_type=DEFAULT_D_TYPE,
                     nodata=0):
        return self._get_image('/tmp/foo.mem', bands, over_sample, data_type,
                               nodata, 'MEM')

    def _get_image(self, file_, bands=1, over_sample=1,
                   data_type=DEFAULT_D_TYPE, nodata=0, driver='GTiff'):
        ...
        transform = [self.eastmin,
                     # self.lower_left[0] ,  # Upper Left X postion
    ...
    @classmethod
    def new_from_envelope(self, envelope_srs, envelope, name=None, geoid=None,
                          **kwargs):
    ...
    def __str__(self):
        return ("AnalysisArea    : {name} \n" +
                ...
                "Scale           : {_scale}\n" +
                "EPGS SRID:      : {srid}\n" +
                "Pro4txt: {proj4txt}").format(proj4txt=self.srs.ExportToProj4(), **self.__dict__)
